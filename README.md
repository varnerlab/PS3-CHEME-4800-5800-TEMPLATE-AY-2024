...
## Problem Description
* We have three streams in the system: stream 1, stream 2, and stream 3. All chemical species can be transported in all streams. No reaction occurs in the streams. Streams 1 and 2 enter the reactor and stream 3 exits the reactor. The [Toy.net](data/Toy.net) reaction file describes the reaction set that occurs in the reactor.
...
* We have the following measurement bounds: $\dot{n}_{A_{1},3}\in[3.0,10.0]$ and $\dot{n}_{A_{2},3}\in[0.0, 6.0]$ while $\dot{n}_{B,3} = 0$. Assume we can sample the species mole flow rates in stream 3 in steps of `0.005`.
...
* After completing the implementation of the `runme()` function, execuute the `runme_task_1.jl` script to estimate the feasible measurement set for the system of linear algebraic equations. The script will save an image of the `frame` array to the `myimages` directory. Once you have generated your test image, you can test it's correctness using the `testme_task_1.jl` script. This script compares the image that you generated with the correct image, and performs a few other checks. If the test script passess all tests, then you have completed the problem successfully.